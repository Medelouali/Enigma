...
2/ Enigma Interpreter:
	...
	2/1/ Operators(Arithemitic+Logical):
			Enigma Interpreter supports all different kind of arithmetic operators that others languages use, 
			and here's a table of all the ops that are supported:
			+, -, /, *, ^, %, //, !, &&, ||, |, &, ~, >, <, >=, <=, ==, !=, =, ++, --, +=, -=
	2/2/ Variables:
		2/2/1/ Naming conventions:
			...
			for example:
			x, value , name, time_0, data_0a are valid Enigma variables while 2_a, 1d are not valid variables 
			so the interpreter will echo an error in such a case.
		2/2/2/ Types:
			2/2/2/1/ Numbers:
					Enigma consider numbers as real numbers and their use is of couse straight forward, for 
				example:
				x=0; degree=100.023; hash=0xfafffaafffa; bool=1;
					Enigma supports a variaty of numbers formats:
						2/2/2/1/1/ Decimals
			...
			2/2/2/2/ Booleans:
				...
				accepted=true; right=false; flag=true;
			2/2/2/3/ Strings
					Most of the time we deal with strings as data type, strings on Enigma are no different from other 
				programming languages, here's how to use them:
				phrase="I got lucky today!"; word='El Ouali is the creator of enigma';
				As you might have noticed, you can use single or double quotes both do the trick.
			2/2/2/4/ Matrices:
					Enigma uses the Matrix data type to hold collection of numerical data and manipulate that data in an easy way here how to use it:
				I=[1, 0; 0, 1]; O=[2, 34; 23 -1]; S=O-I; P=O*I; A=2*I+O;T=*O; D=I/2  
			...
			2/2/2/5/ None:
				...
				noValue=None;
			2/2/2/6/ Objects:
				Last but not least the kings of Object Oriented Programming(OOP), Enigma lets define your own objects, I am sure OOP is big topic but we're not going to dig down in it, that's not the purpose. To use Enigma, knowing JSON is more than enough, here's an example:
				car={"wheels": 4, "milesAge": 2000, "color": "black"}; num=car.wheels; mAge=car.milesAge/9;
		2/2/3/ Variables use:
				Variables are powerfull feature for storing data in any programming language, Enigma lets you 
			manipulate variables in same fashion but in a flexible way:
			t=2.2; M=[-t, t/34; cos(t), sin(cos(t-2))];
			2/2/3/1/ Commands:
				$forget(x);// forget a variable it's no longer in memory;
				$variables;//List a table of all the variables in use so far;
	2/3/ Printing data:
			Unlike other programming languages, you would have to explicitly call a function in order to print 
		a variable to the screen, Enigma does that for you, for example to check the value of X, just throw 
		X into the interpreter and it'll echo back it's value and it goes the same expression like:
		x=21; x;//will throw 21
		f(x)=cos(sin(1-cos(x))); f(f(1-f(2021)));// will throw back the exact value no function needed;
	2/4/ Enigma supports two features of functions:
		2/4/1/ Math Feature:
			This feature is purelly mathematical here's the syntax:
				functionName(variable_1, variable_2, ..., variable_n)=Expression;
				Examples:
				G(p)=1/p; H(x, y)=x^2+y^2-1; S(m)=1/(1-m)+cos(m);
			...
			like:
				*/No Recursion
				**/ One Line Code
				Just a tip, when dealing with mathematical functions and analyzing data, this type is perfect 
				for you but in case of writing more code into your function, consider using the next feature. 
		2/4/2/ Enigma Functions:
			This is the way functions are defined in Enigma, here's the syntax:
			functionName(x){//Some valid enigma code};
	...
	2/6/ Enigma Fundamentals:
			2/6/1/ Variables:
					To define a variable for later use, use the assignement operator = like:
				x=23; y=3994; name="El Ouali"; gotIt=true; m=x-1; t=name + " Mohammed", name=name*23;
				Before you use any variable you must initialize it, otherwise the interpreter will throw 
				an error.
			2/6/2/ Constructing strings with variables:
					Sometimes you need to inject a variable inside of a string, to do that here's the syntax:
				x=2.2; value="the value of x is \${x}";
	

3/ Numerical Computing:
4/ Hacking With Enigma:
5/ Graphs:
`;
